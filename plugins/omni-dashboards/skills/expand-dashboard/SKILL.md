...
allowed-tools: Bash(bash:*), Bash(chart-room:*), Bash(omni:*), Read, Write, Edit, Grep, Glob, AskUserQuestion, Skill
---

# Expand an Omni dashboard

Read @${CLAUDE_PLUGIN_ROOT}/knowledge/workflow-contract.md,
@${CLAUDE_PLUGIN_ROOT}/knowledge/omni-reference.md and
...
Resolve all required deferrals before publication. Browser/Gemini are iteration
requirements; an offline schema operation does not require them.
...
## 5. Author backed tiles in the source
...
Do not relax the schema to get a visualization accepted.
...
State the handoff and actually invoke:

`Skill(skill="omni-dashboards:iterate-dashboard", args="path/to/dashboard.omni.jsonc")`
