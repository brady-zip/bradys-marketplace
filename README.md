...
| [`omni-dashboards`](plugins/omni-dashboards/) | Omni intent → evidence-backed expansion → test rendering → independent Gemini review, with HTML reports and phase ledgers. Five skills plus a Chrome Beta browser agent. Requires an Omni-capable chart-room release; see its [README](plugins/omni-dashboards/README.md) and [acceptance status](plugins/omni-dashboards/ACCEPTANCE.md). |
